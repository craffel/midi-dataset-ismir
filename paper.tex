...
MIDI files that are time-aligned to matching audio can provide ground-truth information  \cite{ewert2012towards, turetsky2003ground}.
...
The motivation behind this project is that MIDI files fit this description.  Through a large-scale web scrape, we obtained 140,910 unique MIDI files -- orders of magnitude larger than any available dataset of aligned transcriptions.
This proliferation of data is likely the result of two factors: First is the wild popularity of karaoke, combined with the widespread use of MIDI files to distribute karaoke tracks, and second that transcribing popular music as MIDI files is a common pastime for hobbyist musicians.
...
Alignment has been studied extensively  \cite{ewert2012towards, turetsky2003ground}, but this work typically assumes that the MIDI and audio have been correctly matched.
...
Thus, the key aspect of our work is a {\em highly-efficient} scheme to match the content of MIDI and files. 
Our system learns a cross-modality hashing which converts both MIDI and audio content vectors to a common Hamming space in which the ``local match'' operation at the core of dynamic time warping (DTW) reduces to a very fast table lookup.
...
Given matched audio and MIDI files, we can use existing alignment techniques to create these alignments, but we must additionally ensure that the alignments are successful; even at the scale of this reduced set of training data, manual alignment verification is impractical, so we develop an improved quality score, described below.
...
Traditional DTW constrains this path to include the start and end of each sequence, i.e. $p[1] = q[1] = 1 and  p[L] = N; q[L] = M$.  However, since our MSD audio consists of cropped preview clips from 7digital, they are usually only subsets of the MIDI.  
We therefore modify this constraint so that either $gN \le p[L] \le N$ or $gM \le q[L] \le M$; $g \approx 1$ is a parameter which provides a small amount of additional tolerance.
We employ an additive penalty $\phi$ for ``non-diagonal moves'', i.e. path entries where either $p[i] = p[i + 1]$ or $q[i] = q[i + 1]$.
...
If we exclude the non-diagonal penalties, the cost of a DTW path $p, q$ through $S$ is calculated by the sum of the distances between the aligned entries of each sequence:
$$
...
$$
We combine the above to obtain a modified DTW cost $c$:
$$
...
Retaining all alignments with costs better than this threshold resulted in 10,035 successful alignments.
...
To maximize the chances of finding a match, we need to use a large and comprehensive pool of audio files; we use the 994,960 7digital preview clips corresponding to the Million Song Dataset, the largest standard research corpus of popular music recordings \cite{schindler2012facilitating}.
A complete search for matches could thus involve 994,960 alignments for each of our 140,910 MIDI files.
If each alignment took 1 millisecond, the full search would take about 4.5 {\em years}.  
Instead, we need a solution in which both computing the distance matrix and calculating the DTW cost for each MIDI/audio pair must be on the order of one hundred {\em micro}seconds.
...
Matching a \textit{single} MIDI file to the MSD using this approach would thus take just under three hours.  Clearly, a more efficient approach is necessary.
...
One objective function for this technique is:
\begin{align*}
...
\end{align*}
where $f$ and $g$ are the nonlinear mappings for each modality, $\alpha$ is a parameter 
to emphasize separating dissimilar items, and $m$ is a target separation of dissimilar pairs.
...
We assemble the set of ``similar'' cross-modalty pairs $\mathcal{P}$ by taking the CQT frames from individual aligned beats in our training set.
...
For the median hash sequence lengths, calculating a Hamming distance matrix between hash sequences is about 400 times faster than computing the CQT cosine distance matrix (24.8 microseconds vs. 9.82 milliseconds) and computing the DTW score is about 9 times faster (106 microseconds vs. 892 microseconds).
...
Even without these improvements, our system can, in a reasonable amount of time, tackle the problem  of resolving our huge MIDI collection against the MSD.
% So did we do it?  Exactly how long will it take?
